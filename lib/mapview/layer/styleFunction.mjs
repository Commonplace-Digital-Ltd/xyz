export default (_xyz) => (layer) => (feature) => {
	...
	if (layer.svgs && properties.count === 1) {
		const pivot = Array.isArray(properties.pivot)
			? properties.pivot[0]
			: properties.pivot;
		const cat = Array.isArray(properties.cat)
			? properties.cat[0]
			: properties.cat;
		const sgvValue = pivot || cat;
		if (cat !== undefined && layer.svgs[sgvValue?.trim()]) {
			style.icon.contribution_svg = layer.svgs[sgvValue?.trim()].replace(
				/'/g,
				`"`
			);
			style.icon.type = 'customSvg';
		}
	}
