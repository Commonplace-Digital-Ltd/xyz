...
function clusterSummary(style, feature) {
  ...
  // if (iOS()) {
  //   let avg = cat.reduce((sum, val) => sum + val, 0) / cat.length;

  //   avg = avg < 12 ? 0 : Math.ceil(+avg / 25) * 25;

  //   const icon = svg.node`
  // <svg width=24 height=24 viewBox='0 0 30 30' xmlns='http://www.w3.org/2000/svg'>
  //   <circle cx=15 cy=15 r=10
  //     stroke="${themeCat[avg].style.strokeColor || '#333'}"
  //     stroke-width="${style.strokeWidth || 1}"
  //     stroke-opacity="${style.strokeOpacity || 1}"
  //     fill="${themeCat[avg].style.strokeColor || 'none'}"></circle>`;
